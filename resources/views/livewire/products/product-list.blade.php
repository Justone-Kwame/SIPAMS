<div class="p-6 space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Manage your product catalogue and inventory.</p>
        </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        {{-- Results count --}}
        <span class="text-sm text-gray-400 ml-auto">{{ $products->total() }} product{{ $products->total() !== 1 ? 's' : '' }}</span>
    </div>
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right">
                            <button wire:click="sortBy('cost_price')" class="flex items-center gap-1 font-bold text-gray-600 hover:text-gray-900 uppercase tracking-wider text-xs ml-auto">
                                Cost
                                @if($sortBy === 'cost_price') <span class="text-teal-500">{{ $sortDir === 'asc' ? '↑' : '↓' }}</span> @endif
                            </button>
                        </th>
                        <th class="px-4 py-3 text-right">
                            <button wire:click="sortBy('selling_price')" class="flex items-center gap-1 font-bold text-gray-600 hover:text-gray-900 uppercase tracking-wider text-xs ml-auto">
                                Price
                                @if($sortBy === 'selling_price') <span class="text-teal-500">{{ $sortDir === 'asc' ? '↑' : '↓' }}</span> @endif
                            </button>
                        </th>
                        ...
                        <th class="px-4 py-3 text-right font-bold text-gray-600 uppercase tracking-wider text-xs">Inv. Value</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($products as $product)
                        @php
                            ...
                            $invValue    = $stockQty * $product->cost_price;
                            $profit      = $product->selling_price - $product->cost_price;
                            $profitPct   = $product->cost_price > 0
                                ? round(($profit / $product->cost_price) * 100, 1) : 0;

                            if ($stockQty == 0) {
                                $stockStatus = ['label' => 'Out of Stock', 'bg' => '#fee2e2', 'color' => '#dc2626'];
                            } elseif ($stockQty <= $product->reorder_level) {
                                $stockStatus = ['label' => 'Low Stock',    'bg' => '#fff7ed', 'color' => '#ea580c'];
                            } else {
                                $stockStatus = ['label' => 'In Stock',     'bg' => '#dcfce7', 'color' => '#16a34a'];
                            }
                        @endphp
                        <tr class="hover:bg-gray-50 transition">
                            {{-- Image --}}
                            <td class="px-4 py-3">
                                <div class="w-10 h-10 rounded-lg overflow-hidden border border-gray-100 flex items-center justify-center bg-gray-50 flex-shrink-0">
                                    @if($product->image_path)
                                        <img src="{{ asset('storage/' . $product->image_path) }}" alt="{{ $product->name }}" class="w-full h-full object-contain">
                                    @else
                                        @php $ci = ord($product->name[0]) % 5; $bgs = ['#e0f7fa','#e8f5e9','#fce4ec','#fff3e0','#f3e5f5']; $fgs = ['#0891b2','#16a34a','#e91e63','#f57c00','#9333ea']; @endphp
                                        <span class="text-base font-black" style="color:{{ $fgs[$ci] }}; background:{{ $bgs[$ci] }}; width:100%; height:100%; display:flex; align-items:center; justify-content:center;">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">
                                    ...
                                    @if($product->brand) · <span class="italic">{{ $product->brand }}</span> @endif
                                </p>
                            </td>
                            ...
                            {{-- Selling Price --}}
                            <td class="px-4 py-3 text-right font-semibold text-gray-800">₵{{ number_format($product->selling_price, 2) }}</td>

                            {{-- Profit --}}
                            <td class="px-4 py-3 text-right">
                                <span class="font-bold {{ $profit >= 0 ? 'text-green-600' : 'text-red-500' }}">
                                    ₵{{ number_format($profit, 2) }}
                                </span>
                                <span class="block text-xs {{ $profitPct >= 0 ? 'text-green-500' : 'text-red-400' }}">
                            ...
                            <td class="px-4 py-3 text-center font-bold text-gray-800">
                                ...
                                <span class="block text-xs font-normal text-gray-400">{{ $product->unit }}</span>
                            </td>

                            {{-- Inventory Value --}}
                            <td class="px-4 py-3 text-right font-semibold text-blue-600">₵{{ number_format($invValue, 2) }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-4 py-12 text-center text-gray-400">
                                ...
                                <p class="text-xs mt-1">Try adjusting your filters or <a href="{{ route('products.create') }}" wire:navigate class="text-teal-600 hover:underline">add a new product</a>.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
            ...
            <h3 class="text-lg font-black text-gray-800 text-center">Delete Product?</h3>
            <p class="text-sm text-gray-500 text-center mt-1 mb-5">This will permanently delete the product and all its batches. This cannot be undone.</p>
